...
export async function registerRoutes(httpServer: Server, app: Express): Promise<Server> {
  ...
  app.get("/api/emails/:id", requireAuth, async (req, res) => {
    try {
      const userId = req.user!.id;
      const email = await storage.getEmail(req.params.id as string, userId);
  ...
  app.patch("/api/emails/:id", requireAuth, async (req, res) => {
    try {
      ...
      const updated = await storage.updateEmail(req.params.id as string, userId, parsed.data);
  ...
  app.delete("/api/emails/:id", requireAuth, async (req, res) => {
    try {
      const userId = req.user!.id;
      const deleted = await storage.deleteEmail(req.params.id as string, userId);
  ...
  app.post("/api/ai/process/:id", requireAuth, async (req, res) => {
    try {
      ...
      const result = await processEmail(req.params.id as string, userId, userDisplayName);
  ...
  app.post("/api/ai/draft-reply/:id", requireAuth, async (req, res) => {
    try {
      ...
      const updated = await storage.updateEmail(req.params.id as string, userId, { aiDraftReply: draft });
      invalidateCache(`briefing:${userId}`);
      res.json(updated);
    } catch (e) {
  ...
  app.post("/api/ai/approve/:id", requireAuth, async (req, res) => {
    try {
      ...
      const updated = await storage.updateEmail(req.params.id as string, userId, { aiDraftReply: null });
      emailContextIndex.invalidate(userId);
      invalidateCache(`briefing:${userId}`);
  ...
  app.delete("/api/folders/:id", requireAuth, async (req, res) => {
    try {
      const deleted = await storage.deleteCustomFolder(req.params.id as string, req.user!.id);
